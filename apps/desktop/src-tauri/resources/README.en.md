...
- **Self-contained now**: runtime-pack layout, manifest contract, bundle globs, assembly wiring, smoke checks, and explicit offline ownership boundaries are in-repo.
- **Still pending through release-time artifact injection**: relocatable Python runtimes, offline wheelhouse contents for OCR/transcription, seeded HuggingFace/PaddleX caches, and audited Linux shared libraries. Lightweight embeddings/NER use OpenRouter and do not require `scripts/embed.py` or spaCy.
